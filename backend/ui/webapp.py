...
from utils import haystack_is_ready, query, send_feedback, upload_doc, haystack_version, get_backlink, facets
...
DEFAULT_QUESTION_AT_STARTUP = os.getenv("DEFAULT_QUESTION_AT_STARTUP", "What is the fibonaecie algoritm?")
...
def main():
    ...
    st.set_page_config(page_title="Intentify", page_icon="https://haystack.deepset.ai/img/HaystackIcon.png")
    ...
    # Title
    st.write("# 🔎💻 Intentify Code Search")
    st.markdown(
        """
Intent based semantic code search
""",
    ...
    input_media = st.sidebar.radio(label="Search with...", options=["text", "image", "intent"])

    filters["$and"]["language"]["$in"] = st.sidebar.multiselect(
        "Programming Language", facets.language, default=facets.language
    )
    ...
    # Run button
    run_pressed = col1.button("Ask Intentify")
